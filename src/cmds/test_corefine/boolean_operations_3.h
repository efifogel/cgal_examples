...
template <typename Polygons,typename EdgeMap, typename NM_edges>
std::size_t connected_components(const Polygons& triangles, const EdgeMap& edge_map,
                                 std::vector<std::size_t>& ccids, NM_edges& nm_edges) {
  ...
  for (std::size_t tid = 0; tid < nb_poly; ++tid) {
    ...
    while(! queue.empty()) {
      ...
      for (int k = 0; k < 3; ++k) {
        // TODO: change that if you want more than triangles
        ...
        switch (tids.size()) {
          ...
          case 2:
          {
            ...
            if (! handled[n_tid]) queue.push_back(n_tid);
          }
          break;
          default:
...
template <typename Concurrency_tag, typename Exact_point, typename Triangle>
Selection_data select(const std::vector<Exact_point>& points, const std::vector<Triangle>& triangles,
                      std::vector<unsigned int>& input_ids, const unsigned int nb_meshes) {
  ...
  std::size_t nb_cc = connected_components(triangles, edge_map, ccids, nm_edges);
  ...
  for (std::size_t i = 0; i < nb_cc; ++i) {
    ...
    CGAL::IO::write_OFF("cc_" + std::to_string(i) + ".off", cc_points, triangles_per_cc[i],
  ...
  for (auto [pid, it] : nm_edges) {
    ...
    for (TID tid : it->second) cc_handled[ccids[tid]] = true;
    ...
    if (triangles[tref.second][(tref.first+1) % 3] != i0) std::swap(i0, i1);
    ...
    auto less = [&tref, &pref, &points, &triangles, &p0, &p1](const std::pair<int, TID>& e1,
      ...
      if (triangles[e1.second][e1.first]==triangles[e2.second][e2.first]) return e1.second < e2.second;
      if (triangles[e1.second][e1.first]==triangles[tref.second][tref.first]) return true;
      if (triangles[e2.second][e2.first]==triangles[tref.second][tref.first]) return false;
    ...
    for (const std::pair<int, TID>& p : to_sort) {
      ...
      debug << 4 << " " << points[triangles[p.second][0]] << " "<< " " << points[triangles[p.second][1]] << " "
            << points[triangles[p.second][2]] << " " << points[triangles[p.second][0]] << "\n";
      debug.close();
    ...
    // coplanar with tref are smaller than the others
    CGAL_assertion(triangles[tref.second][tref.first]!=triangles[to_sort.back().second][to_sort.back().first]);
    ...
    for (std::size_t pos = 0; pos < to_sort.size(); ) {
...
#ifdef CGAL_BO_AUTOREF_DEBUG
            CGAL_assertion(! cc_is_inside_per_mesh_init[mid][ccids[p.second]] ||
      ...
      for (unsigned int mesh_id : mesh_ids) is_inside_mesh.flip(mesh_id);
    }
  }
...
struct Autoref_visitor {
  Autoref_visitor(std::vector<unsigned int>& input_ids, std::size_t nbt1, std::size_t nbt2) :
    input_ids(input_ids),
    nbt1(nbt1),
    nbt2(nbt2)
  {}
...
template <typename Concurrency_tag = Sequential_tag, typename PointRange, typename TriangleRange>
Selection_data prepare_data(const PointRange& points_1, const TriangleRange& triangles_1,
  ...
  for (auto t : triangles_2) {
    for (int i = 0; i < 3; ++i) t[i] += points_1.size();
  ...
  PMP::autorefine_triangle_soup(out_points, out_triangles,
                                CGAL::parameters::concurrency_tag(Concurrency_tag()).visitor(visitor));
#ifdef CGAL_BO3_TIMERS
  time.stop();
...
#endif
#ifdef CGAL_BO_AUTOREF_DEBUG
  CGAL::IO::write_OFF("autorefined.off", points, triangles, CGAL::parameters::stream_precision(17));
#endif
  return select<Concurrency_tag>(out_points, out_triangles, input_ids, 2);
}

template <typename Concurrency_tag = Sequential_tag, typename PointRange, typename TriangleRange>
void update_output(PointRange& out_points,  TriangleRange& out_triangles,
                   std::size_t nbt, const std::vector<bool>& to_keep,
                   const std::vector<bool>& to_flip = std::vector<bool>()) {
  if (nbt == out_triangles.size()) return;
  ...
  for (std::size_t ti = 0; ti < out_triangles.size(); ++ti) {
    if (to_keep[ti]) {
      ...
      t[1] = point_id(t[1]);
      t[2] = point_id(t[2]);
      if (to_flip.empty() || !to_flip[ti]) triangles.push_back(t);
      else {
        std::swap(t[0], t[1]);
...
void compute_union(const PointRange& points_1, const TriangleRange& triangles_1,
  ...
  Selection_data data_out = prepare_data<Concurrency_tag>(points_1, triangles_1, points_2, triangles_2,
                                                          out_points, out_triangles);
