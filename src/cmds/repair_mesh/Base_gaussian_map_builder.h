#ifndef BASE_GAUSSIAN_MAP_BUILDER_H
#define BASE_GAUSSIAN_MAP_BUILDER_H

#include <list>

#include "arr_gaussian_map.h"
#include "make_x_monotone.h"
...
template <typename Vector_3, typename OutputIterator>
OutputIterator Base_gaussian_map_builder::insert(const Vector_3& normal1,
  ...
  *oi++ = (xc2->is_directed_right()) ?
    m_gm.insert_from_left_vertex(*xc2, he->target()) :
    m_gm.insert_from_right_vertex(*xc2, he->target());
  return oi;
}
...
OutputIterator Base_gaussian_map_builder::insert(const Vector_3& normal1,
  ...
  Halfedge_handle he = (xc1->is_directed_right()) ?
    m_gm.insert_from_left_vertex(*xc1, vertex1) :
    m_gm.insert_from_right_vertex(*xc1, vertex1);
  *oi++ = he;
  if (x_objects.size() == 1) return oi;
...
OutputIterator Base_gaussian_map_builder::insert(const Vector_3& normal1,
  ...
  if (x_objects.size() == 1) {
    Halfedge_handle he = (xc1->is_directed_right()) ?
      m_gm.insert_from_right_vertex(*xc1, vertex2) :
      m_gm.insert_from_left_vertex(*xc1, vertex2);
    *oi++ = he->twin();
    return oi;
  }
  ...
  Halfedge_handle he2 = (xc2->is_directed_right()) ?
    m_gm.insert_from_right_vertex(*xc2, vertex2) :
    m_gm.insert_from_left_vertex(*xc2, vertex2);
  he2 = he2->twin();
  Halfedge_handle he1 = (xc1->is_directed_right()) ?
    m_gm.insert_from_right_vertex(*xc1, he2->source()) :
    m_gm.insert_from_left_vertex(*xc1, he2->source());
  he1 = he1->twin();
  *oi++ = he1;
  *oi++ = he2;
  return oi;
