#ifndef CGALEX_COREFINE_INTERSECTION_H
#define CGALEX_COREFINE_INTERSECTION_H
...
/*! computes the intersection of two polygon meshes using corefine and compute intersection.
 */
template <typename Mesh>
void corefine_intersection(Mesh& mesh1, Mesh& mesh2, Mesh& result, std::size_t verbose_level = 0) {
  PMP::Corefinement::Non_manifold_output_visitor<Mesh> visitor(mesh1, mesh2);
  bool is_valid;
  try {
    is_valid = PMP::corefine_and_compute_intersection(mesh1, mesh2, result,
                                                      params::visitor(visitor).throw_on_self_intersection(true));
  }
  catch (const PMP::Corefinement::Self_intersection_exception& e) {
    if (verbose_level > 0) {
      std::cout << "INTERSECTION: Caught an exception: " << e.what() << std::endl;
    }

    soup_intersection(mesh1, mesh2, result);
    return;
  }
  ...
  if (is_valid) {
    ...
    if (verbose_level > 1) {
      if (num_vertices_created > 0) {
        std::cout << "INTERSECTION: Created " << num_vertices_created << " to resolve non-manifold vertices"
                  << std::endl;
      }
      auto self_intersect = PMP::does_self_intersect(result);
      if (self_intersect)
        std::cout << "INTERSECTION: The output self intersect as a result of duplicating non-manifold vertices"
                  << std::endl;
    }
  }
  else {
    if (verbose_level > 0) {
      std::cout << "INTERSECTION: Failed, resorting to handling a non-manifold result\n";
    }
